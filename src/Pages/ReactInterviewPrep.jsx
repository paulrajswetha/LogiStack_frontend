...
import { reactQuestions, topicsList, levelsList } from "./ReactData";
...
export default function ReactInterviewPrep() {
  ...
  const filtered = useMemo(() => {
    return reactQuestions.filter(q => {
  ...
  const counts = useMemo(() => ({
    basic:        reactQuestions.filter(q => q.level === "basic").length,
    intermediate: reactQuestions.filter(q => q.level === "intermediate").length,
    advanced:     reactQuestions.filter(q => q.level === "advanced").length,
  }), []);
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--text-primary)] font-sans">
      ...
      <div className="max-w-[1000px] mx-auto px-5 py-10">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight mb-1">React Interview Questions</h1>
          <p className="text-[var(--text-secondary)] text-sm">
            {reactQuestions.length} questions covering Basic, Intermediate &amp; Advanced topics
          </p>
        </div>
        ...
        {mode === "study" ? (
          <>
            <div className="text-xs text-[var(--text-secondary)] mb-3">
              Showing {filtered.length} of {reactQuestions.length} questions
            </div>
          ...
          <QuizMode questions={filtered.length > 0 ? filtered : reactQuestions} />
        )}
      </div>
    </div>
  );

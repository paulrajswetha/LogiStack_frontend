...
import { sqlQuestions, topicsList, levelsList } from "./SqlData";
    ...
    >
      ...
      {open && (
        <div className="px-5 pb-5 pl-[3.25rem] border-t border-[var(--border-color)]">
          <div className="pt-4 text-[0.85rem] text-[var(--text-secondary)] leading-relaxed whitespace-pre-line font-mono">
            {q.answer}
          </div>
      ...
      <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl overflow-hidden">
        ...
        {!revealed ? (
          <div className="p-6 text-center">
            <p className="text-[var(--text-secondary)] text-sm mb-5">Think of your SQL query, then reveal the answer.</p>
            <button
          ...
          <div className="p-6">
            <div className="text-[0.85rem] text-[var(--text-secondary)] leading-relaxed whitespace-pre-line font-mono mb-6">
              {q.answer}
            </div>
            <p className="text-sm font-semibold mb-3">Did you know this SQL query?</p>
        ...
        {questions.map((_, i) => (
          <div
            ...
            className={`w-1.5 h-1.5 rounded-full transition-colors ${
              i < answers.length
                ? answers[i] === true ? "bg-green-400" : "bg-red-400"
                : i === current
                ? "bg-blue-400"
                : "bg-[var(--border-color)]"
            }`}
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function SQLInterviewPrep() {
  ...
  const counts = useMemo(() => ({
    basic:        sqlQuestions.filter(q => q.level === "basic").length,
    intermediate: sqlQuestions.filter(q => q.level === "intermediate").length,
    advanced:     sqlQuestions.filter(q => q.level === "advanced").length,
  }), []);
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--text-primary)] font-sans">
      ...
      <div className="max-w-[1000px] mx-auto px-5 py-10">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight mb-1">SQL Interview Questions</h1>
          <p className="text-[var(--text-secondary)] text-sm">
            {sqlQuestions.length} questions covering Basic, Intermediate &amp; Advanced topics for 10-20 LPA packages
          </p>
        </div>
        ...
        {mode === "study" ? (
          <>
            <div className="text-xs text-[var(--text-secondary)] mb-3">
              Showing {filtered.length} of {sqlQuestions.length} questions
            </div>
          ...
          <QuizMode questions={filtered.length > 0 ? filtered : sqlQuestions} />
        )}
      </div>
    </div>
  );

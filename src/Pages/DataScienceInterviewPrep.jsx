...
import { dataScienceQuestions, topicsList, levelsList } from "./DataScienceData";
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function DataScienceInterviewPrep() {
  ...
  const filtered = useMemo(() => {
    return dataScienceQuestions.filter(q => {
  ...
  const counts = useMemo(() => ({
    basic:        dataScienceQuestions.filter(q => q.level === "basic").length,
    intermediate: dataScienceQuestions.filter(q => q.level === "intermediate").length,
    advanced:     dataScienceQuestions.filter(q => q.level === "advanced").length,
  }), []);
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--text-primary)] font-sans">
      ...
      <div className="max-w-[1000px] mx-auto px-5 py-10">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight mb-1">Data Scientist Interview Questions</h1>
          <p className="text-[var(--text-secondary)] text-sm">
            {dataScienceQuestions.length} questions covering Basic, Intermediate & Advanced topics
          </p>
        </div>
        ...
        {mode === "study" ? (
          <>
            <div className="text-xs text-[var(--text-secondary)] mb-3">
              Showing {filtered.length} of {dataScienceQuestions.length} questions
            </div>
          ...
          <QuizMode questions={filtered.length > 0 ? filtered : dataScienceQuestions} />
        )}
      </div>
    </div>
  );

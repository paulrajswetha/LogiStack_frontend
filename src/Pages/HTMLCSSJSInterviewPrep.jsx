...
import { htmlCssJsQuestions, topicsList, levelsList, categoriesList } from "./HTMLCSSJSData";
...
const categoryColors = {
  html: { badge: "text-orange-600 dark:text-orange-400 border-orange-500/30 bg-orange-500/10", bg: "bg-orange-500/10" },
  css:  { badge: "text-blue-600 dark:text-blue-400 border-blue-500/30 bg-blue-500/10", bg: "bg-blue-500/10" },
  js:   { badge: "text-yellow-600 dark:text-yellow-400 border-yellow-500/30 bg-yellow-500/10", bg: "bg-yellow-500/10" },
};
...
function StudyCard({ q, index }) {
  ...
  const cc = categoryColors[q.category];
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border capitalize ${cc.badge}`}>
  ...
  if (done) {
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-12">
        ...
        <button
          onClick={restart}
          className="bg-blue-500 text-white px-7 py-3 rounded-xl font-bold hover:opacity-90 transition-opacity"
        >
          Try Again
        </button>
      </div>
      ...
      <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl overflow-hidden">
        <div className="p-6 border-b border-[var(--border-color)]">
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            ...
            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border capitalize ${categoryColors[q.category].badge}`}>
              {q.category.toUpperCase()}
            </span>
            <span className="text-[10px] font-semibold text-blue-600 dark:text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded-full">
              {q.topic}
        ...
        {!revealed ? (
          <div className="p-6 text-center">
            ...
            <button
              ...
              className="bg-blue-500 text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:opacity-90 transition-opacity"
            >
              Reveal Answer
            </button>
          </div>
        ...
        {questions.map((_, i) => (
          <div
            ...
            className={`w-1.5 h-1.5 rounded-full transition-colors ${
              i < answers.length
                ? answers[i] === "knew" ? "bg-green-400" : "bg-red-400"
                : i === current
                ? "bg-blue-400"
                : "bg-[var(--border-color)]"
            }`}
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function HTMLCSSJSInterviewPrep() {
  ...
  const filtered = useMemo(() => {
    return htmlCssJsQuestions.filter(q => {
      ...
      return matchLevel && matchTopic && matchCategory && matchSearch;
    });
  }, [filterLevel, filterTopic, filterCategory, search]);

  const counts = useMemo(() => ({
    basic:        htmlCssJsQuestions.filter(q => q.level === "basic").length,
    intermediate: htmlCssJsQuestions.filter(q => q.level === "intermediate").length,
    advanced:     htmlCssJsQuestions.filter(q => q.level === "advanced").length,
    html:         htmlCssJsQuestions.filter(q => q.category === "html").length,
    css:          htmlCssJsQuestions.filter(q => q.category === "css").length,
    js:           htmlCssJsQuestions.filter(q => q.category === "js").length,
  }), []);
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--text-primary)] font-sans">
      ...
      <div className="max-w-[1000px] mx-auto px-5 py-10">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight mb-1">HTML/CSS/JS Interview Questions</h1>
          <p className="text-[var(--text-secondary)] text-sm">
            {htmlCssJsQuestions.length} questions covering HTML, CSS, and JavaScript
          </p>
        </div>
        ...
        <div className="grid grid-cols-6 gap-2 mb-7">
          <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-2 text-center">
            <div className="text-lg font-extrabold text-orange-400">{counts.html}</div>
            <div className="text-[9px] text-[var(--text-secondary)]">HTML</div>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-2 text-center">
            <div className="text-lg font-extrabold text-blue-400">{counts.css}</div>
            <div className="text-[9px] text-[var(--text-secondary)]">CSS</div>
          </div>
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-2 text-center">
            <div className="text-lg font-extrabold text-yellow-400">{counts.js}</div>
            <div className="text-[9px] text-[var(--text-secondary)]">JS</div>
          </div>
          <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-2 text-center">
            <div className="text-lg font-extrabold text-green-400">{counts.basic}</div>
            <div className="text-[9px] text-[var(--text-secondary)]">Basic</div>
          </div>
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-2 text-center">
            <div className="text-lg font-extrabold text-yellow-400">{counts.intermediate}</div>
            <div className="text-[9px] text-[var(--text-secondary)]">Intermediate</div>
          </div>
          <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-2 text-center">
            <div className="text-lg font-extrabold text-red-400">{counts.advanced}</div>
            <div className="text-[9px] text-[var(--text-secondary)]">Advanced</div>
          </div>
        </div>
        ...
        {mode === "study" ? (
          <>
            <div className="text-xs text-[var(--text-secondary)] mb-3">
              Showing {filtered.length} of {htmlCssJsQuestions.length} questions
            </div>
          ...
          <QuizMode questions={filtered.length > 0 ? filtered : htmlCssJsQuestions} />
        )}
      </div>
    </div>
  );

// DBMSInterviewPrep.jsx
import { useState, useMemo } from "react";
import { dbmsQuestions, topicsList, levelsList } from "./Dbmsdata";
...
export default function DBMSInterviewPrep() {
  ...
  const counts = useMemo(() => ({
    basic:        dbmsQuestions.filter(q => q.level === "basic").length,
    intermediate: dbmsQuestions.filter(q => q.level === "intermediate").length,
    advanced:     dbmsQuestions.filter(q => q.level === "advanced").length,
  }), []);
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--text-primary)] font-sans">
      ...
      <div className="max-w-[1000px] mx-auto px-5 py-10">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight mb-1">DBMS Interview Questions</h1>
          <p className="text-[var(--text-secondary)] text-sm">
            {dbmsQuestions.length} questions covering Basic, Intermediate &amp; Advanced topics
          </p>
        </div>
        ...
        {mode === "study" ? (
          <>
            <div className="text-xs text-[var(--text-secondary)] mb-3">
              Showing {filtered.length} of {dbmsQuestions.length} questions
            </div>
          ...
          <QuizMode questions={filtered.length > 0 ? filtered : dbmsQuestions} />
        )}
      </div>
    </div>
  );
